...
function networkUp() {
  ...
  mkdir ~/final_tprs/raft_4node_swarm
  cp crypto-config/ ~/final_tprs/raft-4node-swarm/
  cp channel-artifacts/ ~/final_tprs/raft-4node-swarm/
...
function networkDown() {
  ...
  docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_CA -f $COMPOSE_FILE_ORG3 down --volumes --remove-orphans
